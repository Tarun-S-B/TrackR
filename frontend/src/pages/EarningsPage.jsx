    ...
    const fetchEarnings = async () => {
      ...
      try {
        const response = await axios.get(
          ...
          }/api/transactions?type=INCOME&page=${page}&limit=10`
        );
  ...
  return (
    <>
      <div className="earnings-page">
        <h1 className="">Earning</h1>
        <div className="flex flex-col gap-4">
          {transactions.map((tx, index) => {
            if (transactions.length == index + 1) {
              return (
                <div ref={lastTransactionElementRef} key={tx.id}>
                  <TransactionEntry transaction={tx} index={index} type="INCOME"/>
                </div>
              );
            } else {
              return (
                <TransactionEntry key={tx.id} transaction={tx} index={index} type="INCOME" />
              );
            }
          })}
        </div>
